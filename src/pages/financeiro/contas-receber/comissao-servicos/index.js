...
import { headerContaReceberComissao } from "../../../../entities/header/financeiro/contas-receber-comissao";

const ComissaoServico = ({ isActive }) => {
  ...
  const limparFiltros = () => {
    ...
    fetchServicos(1, itensPorPagina, termoBusca, {
      status: "",
      data_inicio: "",
      data_fim: "",
      prestador_id: "",
  ...
  const fetchTotaisServicos = async () => {
    try {
      ...
      console.error("Erro ao buscar totais de comissões:", error);
      CustomToast({
        type: "error",
        message: "Erro ao carregar totais das comissões",
      });
    } finally {
      if (isMounted.current) {
  ...
  const salvarParcela = async (parcela, index) => {
    try {
      ...
      CustomToast({
        type: "error",
        message: "Erro ao salvar parcela",
      });
    } finally {
  ...
  const handlePageChange = (newPage) => {
    ...
    fetchServicos(pageNumber, itensPorPagina, termoBusca, {
      status: statusPagamentoFiltro,
      data_inicio: dataInicioFiltro,
      data_fim: dataFimFiltro,
      prestador_id: prestadorFiltro?.id || "",
  ...
  const handleRowsPerPageChange = (newRowsPerPage) => {
    ...
    setPaginaAtual(1);
    fetchServicos(1, newRowsPerPage, termoBusca, {
      status: statusPagamentoFiltro,
      data_inicio: dataInicioFiltro,
      data_fim: dataFimFiltro,
      prestador_id: prestadorFiltro?.id || "",
  ...
  return (
    <>
      {isActive && (
        <div className="w-full flex-col gap-2">
          <div className="flex w-full items-center justify-center gap-2">
            <div
              className="w-full flex-col flex gap-2 items-center justify-center p-2"
            ...
            >
              ...
              <div className="flex items-center w-full justify-between gap-2">
                <div
                  className="w-[32%] flex flex-col gap-1 p-2 justify-center"
                ...
                >
                  ...
                  <label className="text-xs text-center text-primary">
                    {loadingTotais ? (
                      ...
                      formatCurrency(totaisServicos.comissao?.pago || 0)
                    )}
                  </label>
                </div>
                <div
                ...
                >
                  ...
                  <label className="text-xs text-center text-primary">
                    {loadingTotais ? (
                      ...
                      formatCurrency(totaisServicos.comissao?.pendente || 0)
                    )}
                  </label>
                </div>
                <div
                ...
                >
                  ...
                  <label className="text-xs text-center text-primary">
                    {loadingTotais ? (
                      ...
                      formatCurrency(totaisServicos.comissao?.total || 0)
                    )}
                  </label>
                </div>
              </div>
          ...
          <div className="w-full flex justify-center mb-4">
            {loadingTabela ? (
              ...
              <TableComponent
                headers={headerContaReceberComissao}
                rows={servicos}
                actionsLabel={"Ações"}
                actionCalls={{
      ...
      {/* Modal para visualizar parcelas */}
      <ModalLateral
        open={editando}
              ...
              <>
                ...
                <Box className="mb-6 p-4 bg-gray-50 rounded-lg">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <Typography variant="body2">
                      ...
                      {formatCurrency(servicoDetalhado.valor_total)}
                    </Typography>
                    <Typography variant="body2">
                      <strong>Comissão:</strong>{" "}
                      {formatCurrency(servicoDetalhado.comissao)}
                    </Typography>
                    <Typography variant="body2">
                ...
                servicoDetalhado.parcelas.length > 0 ? (
                  servicoDetalhado.parcelas.map((parcela, index) => (
                    ...
                    >
                      <label className="mt-2 mb-2 flex w-full items-center text-xs font-bold gap-2">
                        <Numbers fontSize="small" />
                        Parcela {parcela.numero_parcela}
                      </label>

                      <div className="mt-4 flex gap-3 flex-wrap">
                        {/* Número da Parcela (apenas leitura) */}
                        <TextField
                          fullWidth
                          variant="outlined"
                          size="small"
                        ...
                        {/* Valor da Parcela (apenas leitura) */}
                        <TextField
                          fullWidth
                          variant="outlined"
                          disabled
                          ...
                          value={formatCurrency(parcela.valor_parcela)}
                          InputProps={{
                            readOnly: true,
                            startAdornment: (
                              <InputAdornment position="start">
                        ...
                        <TextField
                          ...
                          disabled
                          value={formatCurrency(parcela.valor_comissao)}
                          InputProps={{
                            readOnly: true,
                            startAdornment: (
                              <InputAdornment position="start">
                        ...
                        >
                          <InputLabel>Status Comissão</InputLabel>
                          <Select
                            value={parcela.status_pagamento_comissao || 1}
                            onChange={(e) =>
                              handleChangeParcela(
                                index,
                ...
                {/* Resumo dos Totais - Apenas Comissão */}
                <Box className="mt-6 p-4 bg-gray-50 rounded-lg">
                  <Typography variant="h6" className="mb-3 font-bold">
                    Resumo de Pagamentos - Comissão
                  </Typography>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <Typography variant="body2">
                      <strong>Comissão Pendente:</strong>{" "}
                      {formatCurrency(servicoDetalhado.total_comissao_pendente)}
                    </Typography>
                    <Typography variant="body2">
                      <strong>Comissão Paga:</strong>{" "}
                      {formatCurrency(servicoDetalhado.total_comissao_pago)}
                    </Typography>
                  </div>
                </Box>
              </>
...
export default ComissaoServico;
